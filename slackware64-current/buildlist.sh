...
find ${1} -type f -exec ls -dl${SIZEOP} --full-time {} \; \
        | awk '{ printf("%10s %2s %4s %4s %8s %10s %-5.5s %s\n",$1,$2,$3,$4,$5,$6,$7,$9) }' > templist.txt

#sort +7d templist.txt >> ${LISTFILE}
sort -k 8d templist.txt >> ${LISTFILE}
rm templist.txt
sed -i '/templist.txt/d' ${LISTFILE}
sed -i '/buildlist.sh/d' ${LISTFILE}


echo "Building \"${CHECKFILE}\" from \"$1\""

if [ -f ${CHECKFILE} ]; then
  rm -f ${CHECKFILE}
fi
find ${1} -type f -exec md5sum {} \; >> ${CHECKFILE}
sed -i '/buildlist.sh/d' ${CHECKFILE}
sed -i "/${CHECKFILE}/d" ${CHECKFILE}
