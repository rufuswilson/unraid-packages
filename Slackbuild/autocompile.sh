...
for d in ${SCRIPTPATH}/*/ ; do
   ...
   find "${SCRIPTPATH}/../slackware64-current/${PACKAGE}/" -type f ! -name "*${VERSIONNEW}*" -exec git rm {} \;
   git add "${SCRIPTPATH}/../slackware64-current/${PACKAGE}/"${PACKAGE}*${VERSIONNEW}*.*
   git commit -m "${PACKAGE} updated to ${VERSIONNEW}"
done
